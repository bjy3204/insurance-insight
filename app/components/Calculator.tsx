...
export default function Calculator() {
  ...
  const onMouseDownDrag = useCallback((e: React.MouseEvent) => {
    if ((e.target as HTMLElement).closest(".calc-btn") || (e.target as HTMLElement).closest(".calc-resize")) return;
  ...
  const btnBase = "flex items-center justify-center rounded-2xl font-medium select-none cursor-pointer transition-all active:scale-95";
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 z-[9998] w-14 h-14 bg-[#2d3250] text-white rounded-full shadow-lg flex items-center justify-center hover:bg-[#1e2340] transition-all active:scale-95"
          title="계산기 열기"
        >
      ...
      {isOpen && initialized && (
        ...
        >
          ...
          {/* 헤더 */}
          <div className="flex items-center justify-between px-5 pt-5 pb-2 cursor-move">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-gray-700 text-sm">보험인사이트 계산기</span>
            </div>
            <div className="flex items-center gap-2">
              <button
              ...
              <button
                ...
                className="calc-btn w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-gray-600 transition-all"
              >
          ...
          {/* 디스플레이 */}
          <div className="mx-4 mb-3 bg-[#f7f7f7] rounded-2xl px-5 py-4 flex flex-col justify-between" style={{ minHeight: 100 }}>
            <div className="text-gray-400 text-sm truncate">{expression || "\u00A0"}</div>
            <div className={`font-light text-right text-gray-900 truncate ${fontSize}`}>{display}</div>
          </div>
